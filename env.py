...
class env:
    ...
    def add_param(self, name, value=None):
        ...
        indices = [index for index, param in enumerate(
            self.params) if param['name'] == name]
        if len(indices) == 0:
    ...
    def create(self):
        ...
        try:
            ...
            for index, param in enumerate(self.params):
                option = input(param["name"] + (" ( " + param["value"] +
                                                " )" if param["value"] is not None else "") + " : ")
                if param["value"] is None and option == '':
                    raise Exception("invalid value to param")
                elif option == '':
                    option = param["value"]
                env.write("{2}{0}=\"{1}\"".format(
                    param["name"], option, "\n" if index != 0 else ""))
        except OSError:
    ...
    def load(self):
        ...
        load_dotenv(envpath)
        # Get the current values - Useful for update process
        try:
